...
from ryu.lib.packet import (                           # noqa: E402
    packet, ethernet, ether_types, ipv4, tcp, udp)
...
class SDNMonitor(AppBase):
    ...
    def _packet_in_handler(self, ev):
        ...
        # Install a flow so subsequent packets are switched by the switch and
        # start accruing the statistics we later poll.
        #
        # IMPORTANT: for IPv4 traffic the rule must carry the IP (and, where
        # applicable, transport-port) fields. The statistics poller can only
        # reason about flows whose match exposes ipv4_src/ipv4_dst, so a
        # MAC-only rule would produce counters the detector has to discard.
        if out_port != ofp.OFPP_FLOOD:
            match = self._build_match(parser, in_port, src, dst, pkt)
    ...
    @staticmethod
    def _build_match(parser, in_port, src, dst, pkt):
        """
        Build the match for a learned flow.

        For IPv4 packets the match includes the IPv4 addresses, the protocol,
        and the TCP/UDP ports where present, so that the per-flow counters the
        switch keeps can be attributed to a real conversation. Anything that is
        not IPv4 falls back to plain L2 matching.

        Including the transport ports means a port scan produces one flow entry
        per probed port, which is exactly the "many tiny flows" signature the
        detector looks for.
        """
        ip4 = pkt.get_protocol(ipv4.ipv4)
        if ip4 is None:
            return parser.OFPMatch(in_port=in_port, eth_dst=dst, eth_src=src)

        fields = dict(in_port=in_port, eth_dst=dst, eth_src=src,
        ...
        if tcp_hdr is not None:
            fields["tcp_src"] = tcp_hdr.src_port
            fields["tcp_dst"] = tcp_hdr.dst_port
        else:
            udp_hdr = pkt.get_protocol(udp.udp)
            if udp_hdr is not None:
                fields["udp_src"] = udp_hdr.src_port
                fields["udp_dst"] = udp_hdr.dst_port
