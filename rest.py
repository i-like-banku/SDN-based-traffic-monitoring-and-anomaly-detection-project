...
REST_HOST = "0.0.0.0"
REST_PORT = int(os.environ.get("SDN_REST_PORT", "8080"))
...
class MonitorRestApp(AppBase):
    ...
    def _serve(self):
        try:
            ...
            self.logger.error(
                "REST server could NOT start: port %s is already in use (%s).",
                REST_PORT, exc)
            self.logger.error(
                "Another program owns that port. Either free it "
                "(sudo ss -tlnp | grep :%s, then kill the PID) or run the "
                "controller on a different port, e.g. "
                "SDN_REST_PORT=8081 python3 run_controller.py", REST_PORT)
            return
